/* reference:
particle.js is taken from Yuxi Chang's previous CS 174A project:
https://github.com/CcccYxx/cs174A-project.git
We also modified it so it fits our need
*/
...
class Particles_Emitter{
    constructor(max_spawn_rate, particle_scale, ave_particle_life, particle_color, max_x_speed, max_y_speed, max_z_speed, outline_enabled) {
    ...
    add_particles(euclidian_model_tranform){
        //ramdom postition at the specified location 
        let model_tranform = euclidian_model_tranform.times(Mat4.translation(1/2*(-1 + 2*Math.random()), 0, 1/2*(-1 + 2*Math.random())));
        ...
        for(let i = 0; i<add_particles_count; i++){
            ...
            if(is_outline && this.outline_enabled){
